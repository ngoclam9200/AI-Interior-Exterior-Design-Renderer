        ...
        <div className="flex-shrink-0 p-3 bg-slate-800/50 border-b border-slate-700 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-white bg-slate-700 hover:bg-slate-600 rounded-md transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
export const ImageEditor: React.FC<ImageEditorProps> = ({ initialImage, onClearInitialImage, onEditComplete, historyItemToRestore, onHistoryRestored }) => {
          ...
          <div className="bg-slate-800/40 p-6 rounded-xl border border-slate-700/50">
            ...
            {image ? (
              <div ref={sourceContainerRef} className="relative group w-full">
                ...
                <canvas ref={sourceDrawingCanvasRef} className="absolute inset-0 w-full h-full object-contain cursor-crosshair rounded-md" onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} />
                <button
                  onClick={() => {
                    setImage(null);
                    setResultImage(null);
            ...
            <>
              <div className="bg-slate-800/40 p-6 rounded-xl border border-slate-700/50">
                <h2 className="text-lg font-semibold text-slate-300 mb-4 flex items-center gap-2">
                  <Icon name="brush" className="w-5 h-5" /> 2. Tùy Chỉnh
                </h2>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  ...
                  <button
                    onClick={handleUndo}
                    disabled={drawingHistory.length === 0}
                    className="w-full bg-slate-700 hover:bg-slate-600 text-white font-semibold py-2 px-2 rounded transition-colors text-sm flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Icon name="arrow-uturn-left" className="w-4 h-4" /> Hoàn Tác
                  </button>
                </div>

                <label htmlFor="brushSize" className="block text-sm font-medium text-slate-400 mb-2">
                  Cỡ Bút: {brushSize}px
                </label>
                <input id="brushSize" type="range" min="5" max="100" value={brushSize} onChange={(e) => setBrushSize(Number(e.target.value))} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                <button onClick={clearMask} className="w-full mt-4 bg-slate-600 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded transition-colors text-sm">
                  Xóa Vùng Chọn
                </button>
              </div>
          ...
          <div className="w-full flex-grow bg-black/20 rounded-lg flex items-center justify-center min-h-[400px]">
            <div className="relative w-full h-full max-w-full max-h-full flex items-center justify-center group">
              {isLoading ? (
                ...
                <p className="text-slate-500 text-center">{image ? "Kết quả sẽ xuất hiện ở đây." : "Vui lòng tải lên một ảnh để bắt đầu."}</p>
              )}
            </div>
          </div>
        </div>
