...
export const VirtualTourTab: React.FC = () => {
  ...
  const handleAction = async (action: string) => {
    ...
    switch (action) {
      case "pan-up":
        prompt = `pan camera up by ${movementDegree} degrees`;
        break;
      case "pan-down":
        prompt = `pan camera down by ${movementDegree} degrees`;
        break;
      case "pan-left":
        prompt = `pan camera left by ${movementDegree} degrees`;
        break;
      case "pan-right":
        prompt = `pan camera right by ${movementDegree} degrees`;
        break;
      case "orbit-left":
        prompt = `orbit camera left by ${movementDegree} degrees`;
        break;
      case "orbit-right":
        prompt = `orbit camera right by ${movementDegree} degrees`;
        break;
      case "zoom-in":
        prompt = `zoom in slightly`;
        break;
      case "zoom-out":
        prompt = `zoom out slightly`;
        break;
      default:
        return;
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" style={{ backgroundColor: "#1c110a" }}>
      ...
      <div className="lg:col-span-1 p-6 rounded-xl flex flex-col gap-6">
        {!sourceImage ? (
          <div className="bg-[#2a1a0e] p-6 rounded-xl border border-slate-700/50">
            ...
            <div className={`relative group border-2 border-dashed rounded-lg p-4 flex items-center justify-center h-48 mb-4 border-slate-600`}>
          ...
          <div className="bg-[#2a1a0e] p-6 rounded-xl border border-slate-700/50 space-y-8">
            ...
            <div className="grid grid-cols-2 gap-3 pt-4">
              <button
                ...
                className="flex items-center justify-center gap-2 p-3 rounded-lg bg-[#5a3a2a] hover:bg-[#6b4a3a] text-white font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
      ...
      <div className="lg:col-span-2 bg-black/30 p-6 rounded-xl flex items-center justify-center min-h-[60vh] lg:min-h-0">
        <div className="w-full h-full flex items-center justify-center relative">
          ...
          {currentImage ? (
            <img src={currentImage} alt="Virtual tour view" className={`max-w-full max-h-full object-contain rounded-md transition-opacity duration-300 ${isLoading ? "opacity-50" : "opacity-100"}`} />
          ) : (
